...
int 		credit2_stop_delay = 0;  /* Tics to delay song stop (0 = no delay) */
jagobj_t	*titlepic;
int			checkit;
...
static boolean credit2_done;
static int credit2_index;
static int credit2_delay;
static int credit2_x;
...
void START_Title (void)
{
    ...
    DoubleBufferSetup ();
    titlepic = (jagobj_t *)W_CacheLumpName ("title", PU_STATIC);
...
void START_Credits2(void)
{
    ...
    backgroundpic = (jagobj_t *)W_POINTLUMPNUM(W_GetNumForName("M_TITLE"));
    DoubleBufferSetup();
    S_StartSong(mus_e1m1, 0);

    /* Load big font sprites (CHAR_097 = 'a', etc.) */
    l = W_GetNumForName("CHAR_097");
    for (i = 0; i < CREDIT_NUMENDOBJ; i++)
...
void DRAW_Credits2(void)
{
    ...
    if (!credit2_done && !--credit2_delay)
    {
        ...
        {
            PrintCreditChar(credit2_text[credit2_index]);
            credit2_delay = CREDIT_TEXT_TIME;
            credit2_index++;
        }
        else
